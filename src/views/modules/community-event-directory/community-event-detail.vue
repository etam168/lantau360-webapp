...
<script setup lang="ts">
  ...
  const onBtnFavClick = () => {
    ...
    if (isCurrentlyFavourite) {
      ...
      const favItem = {
        directoryId: query?.communityEventId,
  ...
  const loadData = async () => {
    if (query?.communityEventId !== undefined) {
      try {
        const [communityEventResponse, galleryResponse] = await Promise.all([
          axios.get(`${COMMUNITY_EVENT_URL}/${query?.communityEventId}`),
          axios.get<GalleryImage[]>(`${COMMUNITY_EVENT_GALLERY_URL}/${query?.communityEventId}`)
        ]);
