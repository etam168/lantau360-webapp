...
<script setup lang="ts">
  ...
  const onBtnFavClick = () => {
    ...
    if (isCurrentlyFavourite) {
      ...
      const favItem = {
        directoryId: props.item?.communityNoticeId,
  ...
  const dialogTitle = computed(() => {
    return translate(
      directoryItem.value.communityNoticeName,
  ...
  const loadData = async () => {
    if (props.item?.communityNoticeId !== undefined) {
      try {
        const [communityNoticeResponse, galleryResponse] = await Promise.all([
          axios.get(`${COMMUNITY_NOTICE_URL}/${props.item?.communityNoticeId}`),
          axios.get<GalleryImage[]>(
            `${COMMUNITY_NOTICE_GALLERY_URL}/${props.item?.communityNoticeId}`
          )
        ]);
