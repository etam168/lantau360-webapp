...
<script setup lang="ts">
  ...
  const isCommunityDirectoryItem = computed(() => isCommunityDirectory(directory));

  const groupBykey = computed(() =>
    isCommunityDirectoryItem.value
      ? "memberId"
      : directory.meta?.groupByKey !== NONE
        ? directory.meta?.groupByKey
        : null
  );
  ...
  const groupedArray = computed(() => {
    if (isCommunityDirectoryItem.value) {
      return [
        { group: "All " + directory.directoryName, items: directoryItemsList },
        {
          group: "My " + directory.directoryName,
          items: (directoryItemsList as PostingView[]).filter(
      ...
      return groupBy(
        directoryItemsList.filter((item: any) => item[key] !== undefined),
        (item: any) =>
          translate(
            item[key],
