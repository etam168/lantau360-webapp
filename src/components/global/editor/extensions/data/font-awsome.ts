...
const FONT_FONTAWSOMEICONS: { [emoticon: string]: string } = {};
const EMOJI_SUGGESTIONS: Emoji[] = [];

const emoticonsForRegex: string[] = [];
...
emojisWithShortcodes.forEach(({ unicode, emoticon, shortcodes }) => {
   ...
   */
  if (emoticon) {
    emoticonsForRegex.push(escapeRegExp(emoticon as string));
    FONT_FONTAWSOMEICONS[emoticon as string] = unicode;
  }
...
export { FONT_FONTAWSOMEICONS, EMOJI_SUGGESTIONS, EMOTICON_REGEX, UNICODE_REGEX };
