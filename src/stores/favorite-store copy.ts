    ...
    if (type === "SITE") {
      if (!favoriteSites.value.some(fav => fav.siteId === (item as SiteView).siteId)) {
        ...
        try {
          ...
          await api.create(ENTITY_URL.FAVOURITE_SITE, payload); // Adjust the endpoint URL if needed
          console.log("Favorite site saved to the database.");
        } catch (error) {
          console.error("Failed to save favorite site to the database:", error);

          // Optionally remove the site if the API call fails
          favoriteSites.value.pop();
          syncLocalStorage("SITE");
        }
      }
    } else if (type === "BUSINESS") {
      if (
        ...
        try {
          ...
          await api.create(ENTITY_URL.FAVOURITE_BUSINESS, payload); // Adjust the endpoint URL if needed
          console.log("Favorite business saved to the database.");
        } catch (error) {
          console.error("Failed to save favorite business to the database:", error);

          // Optionally remove the business if the API call fails
          favoriteBusinesses.value.pop();
          syncLocalStorage("BUSINESS");
        }
      }
    }
